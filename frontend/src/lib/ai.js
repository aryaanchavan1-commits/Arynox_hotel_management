...
const SYSTEM = `You are "Hotel Lakshmi Elite AI", the smart assistant of the Hotel Lakshmi Elite ERP system (hotel + restaurant + POS).
...
function buildSitePrompt(ctx) {
  ...
  return `You are the friendly website assistant of "${ctx.hotel_name || 'Hotel Lakshmi Elite'}".
...
HOTEL FACTS:
Name: ${ctx.hotel_name || 'Hotel Lakshmi Elite'}
Tagline: ${ctx.tagline || ''}
Address: ${ctx.address || ''}
Phone: ${ctx.phone || ''}
Email: ${ctx.email || ''}
...
export async function websiteChat(message, history, ctx, client) {
  const fallback = `Thank you for reaching out to ${ctx.hotel_name || 'Hotel Lakshmi Elite'}!
